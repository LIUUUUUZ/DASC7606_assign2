...
import math
from unet_with_given_label import Unet
...
class Diffuser(nn.Module):
    ...
    @torch.no_grad()
    def sampling(self, n_samples: int, device="cuda", label = []) -> list:
        x_t=torch.randn((n_samples,self.in_channels,self.image_size,self.image_size)).to(device)
        if len(label) == 0:
            label = torch.randint(0, 10, (n_samples,)).to(device)
        else:
            assert len(label) == n_samples
        ...
        for i in range(self.timesteps - 1, -1, -1):
            ...
            x_t = self._reverse_diffusion_with_clip(x_t, t, noise, label)
    ...
    def _forward_diffusion(self, x_0: Tensor, t: Tensor, noise: Tensor) -> Tensor:
        ...
        # ---------- **** ---------- #
        # YOUR CODE HERE
        alpha_cumprod = self.alphas_cumprod.gather(-1, t).reshape(x_0.shape[0], 1, 1, 1)
