...
class Unet(nn.Module):
    ...
    def __init__(self,timesteps,time_embedding_dim,in_channels=3,out_channels=2,base_dim=32,dim_mults=[2,4,8,16]):
        ...
        self.label_embedding=nn.Embedding(10,time_embedding_dim)
    ...
    def forward(self,x,t=None,label=None):
        ...
        for _,block in enumerate(self.encoder_blocks):
            x,x_shortcut=block(x,t_emb,label_emb)
            short_cuts.append(x_shortcut)
        x=self.mid_block(x)
        for i,block in enumerate(self.decoder_blocks):
            x=block(x,short_cuts[-i-1],t_emb,label_emb)
        x=self.final_conv(x)
